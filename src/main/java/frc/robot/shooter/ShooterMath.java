    ...
    public double[] value(double[] parameters) {
      ...
      Vector3d vBall =
          (new Vector3d(
              robotRotation
                  .times(
                      Matrix3d.fromYaw(alpha)
                          .times(Matrix3d.fromPitch(beta).times(new Vector3d(1, 0, 0))))
                  .times(v)));
      ...
      double[] initial = {
        ...
        robotPosition.y(),
        robotPosition.y(),
        robotVelocity.x() + vBall.x(),
        robotPosition.y() + vBall.y(),
        robotPosition.z() + vBall.z()
      };

      FirstOrderIntegrator integrator =
          new DormandPrince853Integrator(1.0e-8, 1.0, 1.0e-10, 1.0e-10);
      ...
      return new double[] {
        finalX - target.x(), finalY - target.y(), finalZ - target.z(), arrivalTheta - theta
      };
    }
  }

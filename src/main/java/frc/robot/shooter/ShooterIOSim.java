...
public class ShooterIOSim implements ShooterIO {
    ...
    public void updateInputs(ShooterIOInputs inputs) {
        ...
        inputs.flywheelVelocityRotationsPerSecond = flywheelVelocityRotationsPerSecond;
        if (flywheelVelocityRotationsPerSecond > 0) {
            double currentTime = Timer.getFPGATimestamp();
            if (currentTime - lastShootTime > .1) {
                Simulator.getInstance().shootBallFromRobot(pitchRadians, yawRadians, flywheelVelocityRotationsPerSecond);
